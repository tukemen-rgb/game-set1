...
団地の航空写真を分析して「リアルさの主成分」を段階的に実装している。

1. **ジオメトリとライティング（実装済み）**: 彫りの深いベランダ
   （床スラブ・手すり・奥の影帯・仕切り）、階段室、パラペットと屋上砂利、
   布団・室外機・雨どい、たるんだ電線、メタセコイア並木、遠景の団地群、
   ゴルフ練習場のネット、曇天モード（`SummerMain.Overcast`）。
   キーアート撮影は `test/KeyArt.cs`（docs/keyart.png）
2. **実写テクスチャ（実装済み）**: [Poly Haven](https://polyhaven.com)（CC0）の
   コンクリ壁・アスファルト・コンクリ床・砂利を `assets/textures/photo/` に取得。
   外壁は雨だれの縦スジを焼き込んで使用（BuildTextures.cs）
3. **生成アセット（未・要APIキー）**: 固定カメラの利点を使い、場面ごとに
   1枚の実写級背景を生成して置き換える「プリレンダー背景」方式が本命。
   プロンプト集は `docs/RESEARCH.md`
...
| 外壁・舗装・屋上 | Poly Haven CC0 実写テクスチャ（assets/textures/photo/）＋雨だれ加工 | 生成テクスチャ |
| 草・葉・田・水面法線ほか | ノイズ＋矩形描画の焼き込み PNG（BuildTextures.cs） | 生成テクスチャ |
