...
### もし画像を公開URLに置けるなら（人手が完全に消えます）

生成画像が**外部から認証なしで取得できるURL**を持てる場合は、
画像のアップロード自体が不要です。`incoming/URLS.txt` に
「ID = URL」を1行ずつ書いてコミットしてください。5分以内に自動で
ダウンロードして取り込みます。

```
BG-danchi-morning = https://example.com/xxxx.png
BG-danchi-noon = https://example.com/yyyy.png
```

- **ID を左**に置いてください（URLに `=` が入るため、区切りは最初の `=`）
- 取得できなければ自動で破棄されるので、**試して失敗しても害はありません**
- **まず1件だけ試して**、取り込まれたか `STATUS.md` で確認してください。
  取り込まれていれば以降の画像は全部この方式で渡せます
