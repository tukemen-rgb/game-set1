...
/// material are corrected; manufactured joints, wall restraint, roof-line offset and ground receiver are
/// explicit geometry. The detail pieces participate in the existing DanchiHighDetail LOD0/1 proxy policy,
...
public static class QualityBlockRainwaterDownpipeInstallationQA
{
    ...
    public static void ApplyAndPersist()
    {
        ...
        foreach (float y in CouplerY)
        {
            GameObject sleeve = CreateCylinder(
                $"HD_DownpipeJointSleeve_{Array.IndexOf(CouplerY, y)}",
                assembly.transform,
                new Vector3(PipeX, y, PipeZ),
                CouplerOuterDiameter,
                CouplerHeight,
                Quaternion.identity,
                pvc,
                false);
        ...
        // Couplers/clamps/offset hardware intentionally survive through LOD1; the macro pipe itself remains
        // outside the accessory LODGroup so LOD2/3 cannot introduce a disappearing-pipe silhouette pop.
    ...
    public static void ValidateOpenScene()
    {
        ...
        if (pipe == null)
        {
            errors.Add("RainGutter macro pipe is missing.");
        }
        else
        {
            ValidatePipe(pipe, pvc, errors);
        }
        ...
        Debug.Log(
            "Rainwater downpipe installation QA passed: physical stand-off, manufactured joints, wall restraint, roof connection, ground receiver and four-level accessory LOD policy are source/runtime-valid. Rendered critical defects remain uncleared until native 4K evidence review.");
    }
    ...
    {
        GameObject band = CreateCylinder(
            $"HD_DownpipeClamp_{index}", root, new Vector3(PipeX, y, PipeZ),
            BandOuterDiameter, BandHeight, Quaternion.identity, metal, false);
        ...
        float armCentreZ = (PipeZ + (FacadePlaneZ + 0.008f)) * 0.5f;
        GameObject arm = CreateBox(
            $"HD_DownpipeStandoff_{index}", root, new Vector3(PipeX, y, armCentreZ),
            new Vector3(0.025f, 0.025f, 0.09f), metal);
        ...
        GameObject plate = CreateBox(
            ...
            new Vector3(PipeX, y, FacadePlaneZ + 0.004f),
            new Vector3(0.075f, 0.065f, 0.008f), metal);
    ...
    private static void BuildRoofOffset(Transform root, Material pvc, Material metal)
    {
        ...
        GameObject collar = CreateCylinder(
            "HD_DownpipeRoofCollar", root,
            new Vector3(PipeX, PipeTop + 0.055f, FacadePlaneZ + 0.010f),
            0.105f, 0.024f, Quaternion.Euler(90f, 0f, 0f), metal, false);
    ...
    {
        GameObject receiver = CreateCylinder(
            "HD_DownpipeGroundReceiver", root,
            new Vector3(PipeX, ReceiverCentreY, PipeZ),
            ReceiverOuterDiameter, ReceiverHeight, Quaternion.identity, pvc, false);
    ...
    private static void ValidateLodPolicy(GameObject detailRoot, List<string> errors)
    {
        ...
        if (!lod0Joint || !lod1Joint || lod2Joint || lod3Joint)
            errors.Add(
                $"Downpipe joint LOD classification invalid: LOD0={lod0Joint}, LOD1={lod1Joint}, LOD2={lod2Joint}, LOD3={lod3Joint}.");
