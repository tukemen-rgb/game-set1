# game-set1 — ニュータウンの夏

**2026年の疲れたサラリーマンが、2000年の夏休みに戻る**ゲーム。
舞台は千里ニュータウンを参考にした架空の1住区（団地・商店街・公園・空き地）。
[Godogen](https://github.com/htdt/godogen) の Godot/Claude パイプライン
（`CLAUDE.md` / `godot.md`）で開発している。

**社長へ**: 留守のあいだの要約は [`docs/WHILE_YOU_WERE_AWAY.md`](docs/WHILE_YOU_WERE_AWAY.md) に1枚でまとめてある。

「夏休みもの」の文法（有限の31日・固定カメラ・小さな遊び・日記）は使うが、
舞台・年代・意匠はオリジナル。経緯は `docs/RESEARCH.md`、
自走開発の判断ログは `docs/AUTOPILOT.md`。

![screenshot](https://raw.githubusercontent.com/tukemen-rgb/game-set1/claude/game-godot-setup-98dhue/docs/screenshot.png)

## できていること

### 物語の骨格

- **タイトル**: 届いた実写キーアート（団地と入道雲）に
  「ニュータウンの なつやすみ／二〇〇〇年 八月」と操作一覧。セーブがあると
  「スペースで つづきから」に変わる
- **導入**: 黒画面から「二〇二六年。会議、見積、終電。」→ 目が覚めると
  セミの声。音だけが先に夏になる構成
- **31日**: 8月1日(火)〜8月31日。1日は8時〜19時（実時間 約3.7分）
- **終盤**: 8/29「しゅくだいの ことは、まだ 考えない。」/ 8/30「あと ふつか。」/
  **8/31「なつやすみ、さいごの 日。」**。日を数えるのは終盤の3日だけ
- **結末**: 2026年へ戻り、その夏に集めたもの（むし・ずかん・場所・はんこ・
  ビー玉）を数え、プレイの結果で最後の一行が変わる。
  導入の「セミの こえが していた」で開き、結末の「セミが 鳴いている
  気がした」で閉じる
- **中断と再開**: 毎朝 自動保存。次の起動は「つづきから はじめる。」。
  夏が終わると記録は消え、また8月1日から

### 遊び

- **あさがおの観察**: 団地の通路に鉢がある。**31日かけて育つ**
  （双葉 → 本葉 → つる → つぼみ → 花）。8/16 以降は朝ごとに咲く数が変わり、
  **咲かない朝もある**。日に一度スペースで見ると、その日の姿が言葉になり
  日記に残る。ラジオ体操が終わる 8/8 以降の「毎朝の一手」
- **ラジオ体操**: 期間中（8/1〜8/7）は台の前に**のぼりが立つ**ので、
  近づく前に「あそこで何かやっている」と分かる。
  **8/1〜8/7 の 8時台**だけ、団地の広場の台で
  はんこを押してもらえる。押した数は夏のあいだ残り、ずかんの下に出る。
  8/7 の朝の台詞は**実際に押した数で変わる**（押さなくても責めない）
- **セミとり**: 6種（ニイニイ / クマ / アブラ / ミンミン / ツクツクボウシ /
  ヒグラシ）。**時間帯で顔ぶれが変わる**ので、時間の経過が遊びに接続する。
  種の出入り（9:30 / 10 / 11 / 13 / 15 / 17 時）では居るセミを動かさず、
  時間の過ぎた種だけ飛び去り、来た種が空いた木に足される。
  種類ごとに捕獲率が違い、ヒグラシ（17時から・40%）が最難関。
  **走って近づくと逃げる**ので、見つけたら歩きに切り替える。歩いても 2.8m 以内では
  毎秒 12% で隣の木へ飛ぶ（止まれば飛ばない）。外すと消えずに **6m 先の木へ飛んで
  とまり直す**。1.2m まで寄ると捕獲率 +0.3。
  変わり目には**新しく出た種を名指しで**知らせる（「アブラゼミが 鳴きはじめた。」、
  去っただけなら「クマゼミの こえが やんだ。」。変化が無ければ黙る）
- **雨の日の生き物**: カタツムリ / アマガエル。雨の日はセミが出ない代わりに
  この2種だけが出る
- **カブトムシ・クワガタ**: 空き地そばの**樹液の出るくぬぎ**にだけ来る。
  **朝（8〜9時半）と夕方（17時以降）だけ**で、昼間は来ない。雨の日も来ない。
  幹の樹液の跡が手がかり
- **ブランコ / すべり台**: ブランコでスペース → 6.5 秒こぐ（その間だけカメラが空を向く）。
  すべり台のはしごの下でスペース → 登って滑る。**土管**の前でスペース → 中に入る（丸い枠、音がこもる。Z で出る）。独白はその最中に出る（立つだけでは言わない）
- **ザリガニつり**: 公園の池のふちに立ってスペース。**垂らす → 待つ →
  「ツン、と ひいた！」で引く**の3拍。早く引くと逃げる。糸を垂らす間は
  虫あみを竿に持ち替え、ウキは水面に浮く。虫あみが
  「近づいて振る」だけなのに対し、待ってから反射で応える手ざわりにした
- **図鑑**: 11種。**Ｚ でいつでも開ける**（時間も止まる）。捕った種は名前、
  まだの種は「？？？」と**手がかりだけ**を見せる。次にどこへ行けばいいかを
  渡すため。日をまたいで残り、未捕獲の個体に近づくと ★ が出る
- **十円玉**: 自販機の下に、**月に6日だけ**十円が落ちている（日付から決まる）。
  発見の独白も、その日 落ちているかで結びが変わる
- **発見**: 世界の8か所で、近づくと独白が出る。
  **8/16 以降に同じ場所へ戻ると、ひと月ぶんの時間が乗った「その後」**が出る
  （1か所につき1回、初めて見つけた日とは別の日に）。
  **締切も失敗も無い**（癒し系で残数を突きつけると焦りになるため）
- **町の人**: 商店街のベンチで新聞を読むおじいさん、団地の広場で立ち話を
  する二人。**それぞれ話しかけられる**（人ごと・日ごとに台詞が変わる）。
  結末で「何回 立ち話をしたか」を数える
- **駄菓子屋**: 商店街で話せる人物。おばあさんの一言は日替わり（8種）。
  **雨の前日は「あしたは 雨が くるよ」と当てる**（本当に翌日が雨の日にだけ言う）。
  雨の日は「あめの 生きものが 出るよ」と教えてくれる。
  1回目のスペースで一言、2回目で品書き。
  話しかけると品書きが開き、**ラムネ(100) / あたりくじ(20) / アイス(50)** を買える。
  おこづかいは毎朝100円（上限900円）。ラムネのビー玉は夏のあいだ残り、
  あたりくじは1/4で当たって40円になる。買った物はその日の日記に残る
- **数を出しっぱなしにしない**: 右上の「むし／ずかん／はっけん／円」は
  **変わったときだけ4秒**出て、あとは消える。ふだんの数はＺのずかんで見る
- **絵日記**: 一日の終わりに、**その日の最後の景色をそのまま貼った1枚**と
  文章がノートのページとして出る。絵は毎日ちがう（実際に見ていた景色なので）。
  達成表にはしない

### 一日と一月の変化

- **木漏れ日**: 木の下の地面に葉の影が落ちる。**晴れの正午が一番濃く、
  朝夕は寝て消える。曇りは薄く、雨は出ない**
- **夕方の灯り**: 17時すぎから、団地と商店街の窓、**アーケードの蛍光灯**、
  自販機の面に灯りが点く。
  **部屋ごとに点く／点かない**が決まっているので、一枚の光る帯にならない
- **時刻**: 空の天頂・地平線の2色を時刻で補間。夕方は金色を経由して夕焼けへ。
  **太陽と環境光の色も時刻で変わる**（14時から効き始め、19時で最も濃い金色）。
  空だけ夕焼けで地面が真昼、という嘘をなくす。
  遠景の実写も同じ光の下に置く
- **天気**: 晴れ / くもり / 雨。**日付から決定的に決まる**（雨は 8/8・17・19・27。
  送り火の 8/16 だけは雨にならないよう翌日と入れ替えてある）。
  雨・くもりの日も時刻で暗くなり、夕方は暖色に寄る。
  雨の日はセミが1匹も出ず、雨音と雨粒に変わる。
  遠景の実写は晴れの1枚しか無いので、**雨・くもりの日は上空を灰色の帯で覆って
  空だけ差し替える**（地平線側は透かして遠景の町を霞ませて残す）。
  雨の日は**地面が濡れる**（暗く・つるりとし、道と広場に水たまりが出る）
- **行事**: 8/7 ラジオ体操最終日 /
  **8/13 お盆（点く窓が 4/10 → 7/10 に増える）** /
  **8/16 送り火（夕方から団地の入口4か所で火を焚く。炎と灯りが揺れる）** /
  **8/24 夏まつり**。当日だけ商店街に**提灯と屋台3軒**が出て、
  品書きが祭りの品（かき氷100 / りんご飴150 / **金魚すくい**200）に入れ替わる。
  18:30から空に花火（明るい空では白い塊になるので暗くなってから）。
  祭りの5日前から日記が数え始める

### 見た目と音

- **固定カメラ6場面**: 団地の棟間（斜め）/ 商店街（望遠20°）/ 公園・池 /
  公園・西（すべり台）/ 大通り / 空き地（土管）。プレイヤー位置で自動切替。
  **主人公が画面の何％になるかを実測して**（`test/CamMetric.cs`）割った
- **遠景の実写**: 場面ごとに専用の写真を貼り替え、昼と夜をクロスフェード
- **主人公**: 腕・脚・靴・目、歩行アニメ（腕は脚と逆位相・歩きと走りで
  歩調と歩幅が変わる）、
  **虫あみ**を持ち、振ると腕が振りかぶってから前へ抜ける
- **町**: 彫りの深い団地（ベランダ・階段室・布団・室外機・雨どい）、
  棟間の広場（舗装・葉のテクスチャを貼った生垣・駐輪場・ゴミ集積所・物干し）、
  町内会の掲示板と立てかけた自転車、
  暖簾と平台のある商店街（端の店は袖看板・室外機・雨どい・貼り紙・
  ビールケースで側面を作り込み、東の入口に門柱／西の端は交差点と家並み）、池とすべり台の公園、
  電線、土管
- **ラジオ体操の朝**: 8/1〜8/7 の 8:00〜8:21、台の前に子ども4人が並んで腕を上げ下げし、
  ラジカセから自作の伴奏が鳴る（主人公との距離で音量が変わる）。はんこを押すと出席カードが出る
- **五時のチャイム**: 17時に町のスピーカーから鐘が4音。
  2000年の団地の夕方は、これが「帰る合図」だった。
  夕方はヒグラシと甲虫が出る時間でもあるので、合図としても働く
- **音**: セミの声を時間帯で3種クロスフェード、雨音、虫あみの
  振る/捕る/逃がす、花火。**すべて標準ライブラリで合成**（外部依存なし）

## まだのもの

- 生成アセットの残り（空2・テクスチャ2・汚し2・キーアート1 = 7枚）
- 設定（音量・速度）
- 届いた雨だれテクスチャが未使用（外壁に重ねる予定）

## アセット表

| アセット | 現状 | 置き換え予定 |
| --- | --- | --- |
| 遠景（4場所 × 昼夜 = 8枚） | **生成済みの実写を使用中** | — |
| タイトルのキーアート | **生成済みの実写を使用中** | — |
| 木漏れ日（葉の影） | **届いた実写を焼き直して使用中** | — |
| 雨だれ | 焼き直し済み・未使用 | 外壁へ |
| 団地・商店街・公園・道路の造形 | プロシージャル（BuildSummer.cs）。公園は GPT の参考画 `docs/reference/park_pond_gpt.png` に合わせて再現 | GLBモデル |
| 池のふちの石張り / 粗い芝 | `tools/gen_textures.py` で合成（`assets/textures/gen/cobble.png`, `grass_coarse.png`） | 実写テクスチャ |
| 外壁・舗装・屋上 | Poly Haven CC0 実写＋雨だれ加工 | 生成テクスチャ |
| 草・葉・田・水面法線ほか | ノイズ焼き込み PNG（BuildTextures.cs） | 生成テクスチャ |
| 音（セミ3種・雨・効果音4種） | 標準ライブラリで合成（gen_audio.py） | — |
| 主人公・おばあさん・セミ | プリミティブ組み合わせ | リグ付きモデル |

## 遊び方

| 操作 | キー |
| --- | --- |
| 歩く | 矢印キー |
| 走る | Shift ＋ 矢印キー |
| 虫あみ / つり / 話す / 決定 | スペース / Enter |
| ずかん / 品書きを やめる | Ｚ |

既定は**歩き**（2.6 m/s）。急ぐときだけ走る（4.5 m/s）。
のんびり眺める遊びなので、速いほうを既定にしない。
**走っていると 4m 以内の虫は逃げる**ので、速さには代償がある。

## 開発環境と実行

必要なもの: **Godot 4.4 (.NET 版)** と **.NET SDK 8**。

コンテナを作り直すと Godot は消えるので入れ直す（`.NET SDK` は `/opt/dotnet`）。
**`DOTNET_ROOT` を渡さないと `Failed to load hostfxr` で落ちる**（無言ではないが
原因が分かりにくい）。

```bash
curl -sSL -o /tmp/godot.zip https://github.com/godotengine/godot/releases/download/4.4-stable/Godot_v4.4-stable_mono_linux_x86_64.zip
unzip -q /tmp/godot.zip -d /opt/godot
ln -sf /opt/godot/Godot_v4.4-stable_mono_linux_x86_64/Godot_v4.4-stable_mono_linux.x86_64 /usr/local/bin/godot
export DOTNET_ROOT=/opt/dotnet PATH="$PATH:/opt/dotnet"
```

```bash
dotnet build                                                     # C# コンパイル
godot --headless --path . --script res://scenes/BuildTextures.cs # テクスチャ生成
python3 tools/gen_audio.py                                       # 音の合成
godot --headless --import                                        # 生成物の取り込み
godot --headless --path . --script res://scenes/BuildSummer.cs   # シーン再生成
godot --path .                                                   # 実行
```

シーンは手書きせず `scenes/BuildSummer.cs` が `scenes/summer_main.tscn` を
出力する（`godot.md` の規約）。**取り込み後の `--import` を飛ばすと、
`ResourceLoader.Exists` が false を返して機能が無言で消える**ので注意。

## 検査（キャプチャ）

目視検査用のスクリプトを `test/` に置いている。すべて `SkipIntro` を立てるので、
**検査が人のセーブを壊さない**。

| スクリプト | 見るもの |
| --- | --- |
| `Presentation.cs` | 通しプレイ（決まった動線・乱数固定） |
| `Intro.cs` / `Ending.cs` | 導入 / 結末（導入は `INTRO_FRESH=1` でセーブを消して流す） |
| `CloseUp.cs` | 寄り。`CLOSEUP_WALK=1` 歩行 / `CLOSEUP_SWING=1` 虫あみ |
| `CamShots.cs` | 固定カメラ6場面すべて（`SHOT_DAY` / `SHOT_HOUR`）/ `KeyArt.cs` 俯瞰 |
| `CamMetric.cs` | 各ゾーンで主人公が画面の何px・何％になるかの実測 |

`CamShots.cs` は `SHOT_DAY=8`（雨の日）と `SHOT_HOUR=12.5`（時刻）を指定できる。
| `Weather.cs` | 天気（`CLOSEUP_DAY` で日付指定） |
| `Festival.cs` | 夏まつりの花火 / `Shop.cs` 駄菓子屋の会話と買い物 |
| `SaveCheck.cs` | 中断と再開 / `SeedProbe.cs` 乱数シード探索 |
| `Fishing.cs` | ザリガニつり（当たりの時刻は乱数なので画面の文言で待つ） |
| `Dex.cs` | ずかん（2種捕ってから開く） |
| `Radio.cs` | ラジオ体操のはんこ（8時台に台の前へ） |
| `Coin.cs` | 自販機の下の十円（`COIN_DAY=9` 落ちている日 / `10` 無い日） |
| `Granny.cs` | おばあさんの一言（`GRANNY_DAY=7` 雨の前日 / `8` 雨の日） |
| `Startle.cs` | 走ると虫が逃げる（`STARTLE_WALK=1` で歩きと比較） |
| `Revisit.cs` | 発見した場所へ翌日戻ると「その後」が出る |
| `Talk.cs` | 町の人との立ち話 |
| `Swing.cs` / `Slide.cs` / `Pipe.cs` | ブランコに乗る / すべり台を登って滑る / 土管に入る。`ParkView.cs` 公園を歩く |
| `Month.cs` | **8/1〜結末を一気に走破**して文・行事・日記を全部ログに流す（ヘッドレス約15分。`MONTH_START=n`） |
| `AudioProbe.cs` | 音素材の形式・長さ・ループ終端を数字で出す（ヘッドレス） |

```bash
bash tools/shoot_all.sh              # 全場面（30分ほど）
bash tools/shoot_all.sh cams dex     # 触った所だけ
bash tools/make_video.sh run docs/video/summer_day.mp4 4 20   # 連番PNG＋音 → 動画
python3 tools/check_audio.py screenshots/run                  # 音量を1秒ごとに測る
```

`screenshots/` には `.gdignore` を置いてある。無いと Godot が撮ったPNGを
**全部インポートしようとして** `.godot/imported` が数GBまで膨らみ、
`--import` が終わらなくなる（実際 7.4GB / 29700ファイルになっていた）。

```bash
```

**動いているところ**（音つき）:
[`docs/video/summer_day.mp4`](docs/video/summer_day.mp4)（20秒・8月1日の一日）/
[`docs/video/summer_festival.mp4`](docs/video/summer_festival.mp4)（18秒・8月24日の夏まつり）/
[`docs/video/park_pond.mp4`](docs/video/park_pond.mp4)（16秒・参考画に合わせた公園）

### 監査レポート

`docs/audit/` に 2026-09-02 の総点検（音 / コードレビュー / 31日走破 / 視覚スイープ）の
報告がある。各項目は「何が / 証拠 / 原因 / 直し方」で書かれ、直したものは
`docs/AUTOPILOT.md` のイテレーション 68〜71 に対応する。

### 検査用の環境変数

「撮れないものは直せない」ので、見たい状態を作るスイッチはここに集める。

| 変数 | 効果 |
| --- | --- |
| `SHOT_DAY` / `SHOT_HOUR` | `CamShots.cs` の日付・時刻（雨の日や正午を撮る） |
| `CLOSEUP_DAY` / `CLOSEUP_AT` / `CLOSEUP_EYE` | `CloseUp.cs` の日付・対象・寄る向き |
| `CLOSEUP_WALK` / `CLOSEUP_SWING` | 歩行 / 虫あみを振る姿を撮る |
| `INTRO_FRESH=1` | `Intro.cs` でセーブを消してから導入を流す（**消す**） |
| `FISH_CLOSEUP=1` | `Fishing.cs` で竿と糸に寄る |
| `DEBUG_SPAWN=1` | その日どこに何が湧いたかを出す |
| `DEBUG_FIREWORK=1` | 花火の打ち上げ位置を出す |
| `DEBUG_STEPS=1` | 足音が鳴った回数を出す |
| `DEBUG_MSG=1` | 画面に出た文を出た順に流す（`[msg~]` 待ち行列へ / `[msg+]` 行列から / `[msg-]` 捨てた / `[msg!]` 上書き） |
| `DEBUG_UNLIT=1` | 光が当たらない材質のうち、時刻の色掛けの対象外のものを列挙 |
| `END_DAY` | `Ending.cs` の開始日（`END_DAY=30` で最終日の朝を見る） |
| `SpeedProbe.cs` | 歩き／走りの移動距離の実測（ヘッドレス） |

```bash
xvfb-run -a godot --path . --write-movie screenshots/run/frame.png \
  --fixed-fps 30 --quit-after 700 --script res://test/Presentation.cs
```

## 画像生成の連携

GPT に背景などを作ってもらい、`incoming/` 経由で取り込む仕組みがある。

- `docs/prompts/PROMPTS.md` プロンプト集（113件。うち★15件が優先 → **15/15 完了**）
- `docs/prompts/STATUS.md` 入荷状況（自動更新）
- `docs/prompts/GPT_INSTRUCTIONS.md` GPT に貼る指示書
- `tools/ingest_images.sh` 取り込み（`MAP.txt` / `URLS.txt` に対応）
- `.github/workflows/generate-images.yml` API キーがあれば1時間ごとに自動生成

## 構成

```
scenes/BuildSummer.cs    シーンビルダー（summer_main.tscn を生成）
scenes/BuildTextures.cs  プロシージャルテクスチャ
scripts/SummerMain.cs    進行管理（日数・天気・行事・カメラ・音・セーブ）
scripts/PlayerController.cs  移動と歩行アニメ
tools/gen_audio.py       音の合成 / tools/gen_prompts.py プロンプト集の生成
tools/gen_decals.py      届いた汚しを重ね用（色＋アルファ）に焼き直す
docs/AUTOPILOT.md        自走開発の判断ログ（何をなぜ直したかの記録）
```
